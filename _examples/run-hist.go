...
func (s GnuplotStyle) DrawPlot(p *plot.Plot, c vgdraw.Canvas) {
	...
	xda := plot.PadX(p, c.Crop(ywidth-y.Width-y.Padding, 0, 0, 0))
	yda := plot.PadY(p, c.Crop(0, xheight-x.Width-x.Padding, 0, 0))
	...
	datac := plot.PadY(p, plot.PadX(p, c.Crop(ywidth, xheight, 0, 0)))
