...
func TestTypstCanvas(t *testing.T) {
	cmpimg.CheckPlot(Example, t, "scatter_"+runtime.GOARCH+".typ")
}

func TestLineTypst(t *testing.T) {
	...
	cmpimg.CheckPlot(test("testdata/linestyle_"+runtime.GOARCH+".typ"), t, "linestyle_"+runtime.GOARCH+".typ")
...
func TestFillStyle(t *testing.T) {
	cmpimg.CheckPlot(func() {
		...
		const size = 5 * vg.Centimeter
		err = p.Save(size, size, "testdata/fillstyle_"+runtime.GOARCH+".typ")
		if err != nil {
			t.Fatalf("error: %+v", err)
		}
	}, t, "fillstyle_"+runtime.GOARCH+".typ")
}
