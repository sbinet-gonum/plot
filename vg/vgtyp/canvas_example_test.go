...
import (
	...
	"gonum.org/v1/plot/vg/vgtyp"
)

// An example of making a Typst plot.
func Example() {
	p := plot.New()
	// p.HideAxes()
	p.Title.Text = `A scatter plot: $sqrt((e^(3i pi))/(2 cos 3 pi))$`
	...
	c := vgtyp.New(5*vg.Centimeter, 5*vg.Centimeter)
	p.Draw(draw.New(c))

	c.SetColor(color.Black)
	c.FillString(txtFont, vg.Point{X: 2.5 * vg.Centimeter, Y: 2.5 * vg.Centimeter}, "x")

	f, err := os.Create("testdata/scatter_" + runtime.GOARCH + ".typ")
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()
